...
public class Quick_Responses_Approver_CreateandVerifyResponse_Test extends BaseClass {
	...
	@Test(priority = 3, dependsOnMethods = "AddQuickResponse", description = "Test to verify added Response")
	public void VerifyResponseAdded() throws Exception {
		ExcelHandler wb = new ExcelHandler("./data/Reviews.xlsx", "Quick Response");
		String quickResponse = wb.getCellValue(1, wb.seacrh_pattern("AddResponse", 0).get(0).intValue());
		System.out.println("The quick response to be added is : " +quickResponse);
		data = new Quick_Responses(CurrentState.getDriver());
		data.verifyResponse(quickResponse);
	}
	
	@Test(priority = 4, dependsOnMethods = "VerifyResponseAdded", description = "Test to delete added Response")
