...
public class ContentManagement_Test extends BaseClass {
	...
	public void searchwithStatus() throws Exception {
		...
		for (row = 1; row <= wb.getRowCount(); row++) {
			...
			if (!Status.equalsIgnoreCase("null")) {
				...
				data.VerifystatusinTable(Status);
	...
	/**
	 * To Approve the post based on status and Type
	 * 
	 * @throws Exception
	 */
	@Test(priority = 12)
	public void ApprovePost() throws Exception {
		data = new ContentManagement_Page(CurrentState.getDriver());
		wb = new ExcelHandler("./data/Social_Engagement.xlsx", "SE");
		wb.deleteEmptyRows();
		int row;
	...
	/**
	 * To Reject the post based on status and Type
	 * 
	 * @throws Exception
	 */
	@Test(priority = 13)
	public void RejectPost() throws Exception {
		data = new ContentManagement_Page(CurrentState.getDriver());
		wb = new ExcelHandler("./data/Social_Engagement.xlsx", "SE");
		wb.deleteEmptyRows();
		int row;
		for (row = 1; row <= wb.getRowCount(); row++) {
			...
			System.out.println("The Status is :" + Status);
			if ((Status.equalsIgnoreCase("Pending Approval"))) {
				data.RejectPost(Type, Status);
				addEvidence(CurrentState.getDriver(), "Post Approval", "yes");
				CurrentState.getDriver().navigate().refresh();
			} else {
				System.out.println("Cannot Reject the post with given Type and Status");
			}
		}
	}

	/**
	 * To Approve the post based on Type
	 * 
	 * @throws Exception
	 */
	@Test(priority = 12)
	public void ApprovePostusingType() throws Exception {
		data = new ContentManagement_Page(CurrentState.getDriver());
		wb = new ExcelHandler("./data/Social_Engagement.xlsx", "SE");
		wb.deleteEmptyRows();
		int row;
	...
	/**
	 * To Reject the post based Type
	 * 
	 * @throws Exception
	 */
	@Test(priority = 15)
	public void RejectPostusingType() throws Exception {
		data = new ContentManagement_Page(CurrentState.getDriver());
		wb = new ExcelHandler("./data/Social_Engagement.xlsx", "SE");
		wb.deleteEmptyRows();
		int row;
	 ...
	 */
	@Test(priority = 16)
	public void ApprovePostusingwith3Criteria() throws Exception {
		data = new ContentManagement_Page(CurrentState.getDriver());
		wb = new ExcelHandler("./data/Social_Engagement.xlsx", "SE");
		wb.deleteEmptyRows();
		int row;
		for (row = 1; row <= wb.getRowCount(); row++) {
			...
			String Status = wb.getCellValue(row, wb.seacrh_pattern("Unpublished Status", 0).get(0).intValue());
			System.out.println("The Status is :" + Status);
			String Key = wb.getCellValue(row, wb.seacrh_pattern("Unpublished Search", 0).get(0).intValue());
			System.out.println("Search key is :" + Key);
			if ((Status.equalsIgnoreCase("Pending Approval"))) {
				data.ApprovePost3Criteria(Type, Status, Key);
				addEvidence(CurrentState.getDriver(), "Post Approval", "yes");
				CurrentState.getDriver().navigate().refresh();
			} else {
				System.out.println("Cannot Approve the post with selected Status");
			}
		}
	}
	 ...
	 */
	@Test(priority = 17)
	public void RejectPostwith3Criteria() throws Exception {
		data = new ContentManagement_Page(CurrentState.getDriver());
		wb = new ExcelHandler("./data/Social_Engagement.xlsx", "SE");
		wb.deleteEmptyRows();
		int row;
		for (row = 1; row <= wb.getRowCount(); row++) {
			...
			System.out.println("Search key is :" + Key);
			if ((Status.equalsIgnoreCase("Pending Approval"))) {
				data.RejectPost3Criteria(Type, Status, Key);
				addEvidence(CurrentState.getDriver(), "Post Approval", "yes");
				CurrentState.getDriver().navigate().refresh();
			} else {
				System.out.println("Cannot Reject the post with selected Status");
			}
		}
	}
