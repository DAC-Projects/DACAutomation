...
public class TPSEE_ContentAnalysis_Page extends TPSEE_abstractMethods{
	...
	public List<Map<String, String>> getExportData() throws Exception {
		...
		for (int col = 1; col < colSize; col++) {
			...
			exportData.add(kMap);//  added by aneesh
		}		
	...
	public String[][] getExportDataNew() throws Exception {
		JSWaiter.waitJQueryAngular();
		exportcontentanalysisrpt();
		String[][] table = new ExcelHandler(Exportpath + (CurrentState.getBrowser()+ContentAnalysisExport), "Sheet0").getExcelTableNew();
		return table;
	}
		...
		if(siteTable.isDisplayed()){
			...
			for (int row = 0; row < rows_count; row++) { 
				...
				for (int column = 0; column < columns_count; column++) {	//Loop will execute till the last cell of that specific row.
					...
					//	System.out.println("map value"+kMap.get("rowdata"));
	...
	public void compareExprttoAnalysisSiteData(List<Map<String, String>> exportData,
		...
		for (Map<String, String> m1 : exportData) {
			for (Map<String, String> m2 : analysisSiteData) {
				...
				System.out.println("m1 data"+m1.get("rowdata"));
				System.out.println("m2 data"+m2.get("rowdata"));
	...
	public String[][] getSiteData(){			
		...
		if(siteTable.isDisplayed()){
			...
			int columns_count  = rows_table.get(1).findElements(By.tagName("td")).size();//To locate columns(cells) of that specific row.

			//	System.out.println("col Count : "+columns_count);

			//data = new String[rows_count+1][columns_count-1];
			data = new String[rows_count+1][columns_count];
			String[] cellValues ;    	

			//For header
			cellValues = new String[columns_count];	 
			for (int column = 0; column < columns_count; column++) {	//Loop will execute till the last cell of that specific row.
				List<WebElement> headerTableRow=SiteTableHeader.findElements(By.tagName("th"));
				cellValues[column] = headerTableRow.get(column).getText();
				//System.err.println("Heder value"+headerTableRow.get(column).getText());
			}
			data[0] = cellValues;
			...
			for (int row = 1; row <= rows_count; row++) { 
				List < WebElement > Columns_row = rows_table.get(row-1).findElements(By.tagName("td"));//To locate columns(cells) of that specific row.
				//To calculate no of columns (cells). In that specific row.
				cellValues = new String[columns_count];	 
				for (int column = 0; column < columns_count; column++) {	//Loop will execute till the last cell of that specific row.

					if(column==0) {
						cellValues[column] = driver.findElement(By.xpath("(//*[@class='logo-img img-responsive sourceImg'])["+ (row) +"]")).getAttribute("id");
						//		vendorname= driver.findElement(By.xpath("(//*[@class='logo-img img-responsive sourceImg'])["+ (row+1) +"]")).getAttribute("id");
						//		celtext = driver.findElement(By.xpath("(//*[contains(@class,'easy-pie-chart percentage easyPieChart')])["+ (row+1) +"]")).getAttribute("data-percent");
					}else {
						cellValues[column]=	Columns_row.get(column).getText().split("\\r?\\n")[0].replace(".00", "");
						//	cellValues[column] = driver.findElement(By.xpath("(//*[contains(@class,'easy-pie-chart percentage easyPieChart')])["+ (column+1) +"]")).getAttribute("data-percent");
		...
		System.out.println(" ui array sze"+data.length+" "+data[0].length);
		return data;
